...
- The builtin `host-mcp-manager` plugin registers a host MCP server that can list, install, and remove persisted host MCP servers, prompt the user to store secrets in the host vault, install the repo-local `rest-api` stdio MCP server, install local plugin bundles from the managed guest workspace into the desktop user's plugin directory, and inspect plugin-declared `camelai.agentAssets`. Those bundled skills are reconciled declaratively into the Codex, Claude, PI, and OpenCode runtime homes when plugins are loaded or refreshed; bundled MCP configs are reconciled into the Codex and Claude runtime homes.
