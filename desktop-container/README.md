...
- `desktop-container/container-images/` contains the shared Apple-container image definition that installs ACPX, Codex, and Claude Code together, plus the `acon-host-bridge` daemon and `acon-host-rpc` CLI so agents can call host RPC from inside the container.
