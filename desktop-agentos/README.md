...
- `desktop-agentos/backend/extensions/host.ts` discovers V2 `camelai` plugin manifests from `desktop-agentos/plugins/builtin/` plus the user install directory, loads extension modules, exposes a runtime-first API (`on`, `registerPage`, `registerPreviewPane`, `registerCommand`, `registerTool`), and materializes desktop pages plus chat-owned preview panes into the shared snapshot model.
- `desktop-agentos/backend/extensions/thread-state.ts` provides a persistent per-thread plugin state store backed by the shared `.pi` home so host pages, preview panes, and runtime hooks can share thread-scoped JSON state.
...
- `desktop-agentos/plugins/` contains repo-shipped V2 plugins, with `plugins/builtin/` reserved for curated builtins. The current builtin set includes `extension-lab`, `random-site-preview`, and `thread-journal`.
...
- The shared desktop renderer (`desktop/renderer/src/App.tsx`) now treats plugin entries as full-page main-area navigation targets. Chat remains thread-driven, and the right-hand companion pane is stored per thread so chat-owned previews can open without changing the selected plugin page. Host-rendered plugin pages are live, and plugin-owned webviews can render `http:`, `https:`, `data:`, and plugin-local HTML entrypoints in a sandboxed iframe via the desktop file/webview bridge.
